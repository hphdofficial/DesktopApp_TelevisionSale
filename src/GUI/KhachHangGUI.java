...
public class KhachHangGUI extends JPanel implements MouseListener, FocusListener, DocumentListener, KeyListener {
    ...
    public void init() {
        ...
        lbIMG.setIcon(new ImageIcon("./img/imgCustomer.png"));
...
//            }
    ...
    public JPanel panelInfo() {
        ...
        int toadoxLabel = 270, toadoyLabel = 40;
        int toadoxTextField = 420, toadoyTextField = 40;
...
//        p.add(btnIMG);
    ...
    public JPanel panelSearch() {
        ...
        JLabel lblSortMaKH = new JLabel("Mã khách hàng :");
        lblSortMaKH.setFont(font0);
        lblSortMaKH.setBounds(30, 100, 90, 30);
        p.add(lblSortMaKH);

        sortMaKH = new JTextField();
        sortMaKH.setFont(font0);
        sortMaKH.setBounds(new Rectangle(130, 100, 150, 30));
        sortMaKH.addKeyListener(this);
        p.add(sortMaKH);

        /********** SORT HOKH **********/

        JLabel lblSortLoai = new JLabel("Họ khách hàng :");
        lblSortLoai.setFont(font0);
        lblSortLoai.setBounds(30, 140, 90, 30);
        p.add(lblSortLoai);

        sortHoKH = new JTextField();
        sortHoKH.setFont(font0);
        sortHoKH.setBounds(new Rectangle(130, 140, 150, 30));
        sortHoKH.addKeyListener(this);
        p.add(sortHoKH);

        /********** SORT TENKH **********/

        JLabel lblSortNSX = new JLabel("Tên khách hàng :");
        lblSortNSX.setFont(font0);
        lblSortNSX.setBounds(30, 180, 90, 30);
        p.add(lblSortNSX);

        sortTenKH = new JTextField();
        sortTenKH.setFont(font0);
        sortTenKH.setBounds(new Rectangle(130, 180, 150, 30));
        sortTenKH.addKeyListener(this);
        p.add(sortTenKH);
...
//        try {
//            if (image != null) {
//                File save = new File("./img/KhachHang/" + imgName);    //  Tạo file
...
//    }
    ...
    public void search() {
        String makh = sortMaKH.getText();
        String ho = sortHoKH.getText();
        String ten = sortTenKH.getText();
        String gt = sortGT.getSelectedIndex() != 0 ? sortGT.getSelectedItem() : "";

        outModel(model, khBUS.search(makh, ho, ten, gt));
    }
...
//            Image newImage;
//            try {
//                newImage = new ImageIcon("./img/KhachHang/" + imgName).getImage().getScaledInstance(200, 200, Image.SCALE_DEFAULT);
//            } catch (NullPointerException E) {
//                newImage = new ImageIcon("./img/KhachHang/NoImage.jpg").getImage().getScaledInstance(200, 200, Image.SCALE_DEFAULT);
//            }
        ...
        if (e.getSource() == btnDelete) {
            ...
            if (messDelete == 0) {
                khBUS.delete(textfields[0].getText());
                table.clearSelection();
                cleanView();
                listKH();   //  outModel(model, khBUS.getList());
...
//        }
        ...
        if (e.getSource() == btnConfirm) {
            if (AddOrEdit) {    // Add
                ...
                if (messEdit == 0) {
                    ...
                    KhachHangDTO kh = new KhachHangDTO(maKH, hoKH, tenKH, gioiTinh, dienThoai, diaChi);
                    khBUS.update(kh);
                    listKH();   //  outModel(model, khBUS.getList());   // Load lại table
                    cleanView();
        ...
        if (e.getSource() == btnImport) {
            ...
            if (result == JFileChooser.APPROVE_OPTION)
            {
                ...
                khBUS.ImportExcelDatabase(file);
                khBUS.list();
                outModel(model, khBUS.getList());
