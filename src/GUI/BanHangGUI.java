...
public class BanHangGUI extends JPanel implements MouseListener, KeyListener, ActionListener {
    ...
    public BanHangGUI(int width, int height, String userID) {
        this.userID = userID;
        Default_Width = width;
    ...
    public void init() {
        ...
        lbIDBill = new JLabel("Mã hóa đơn");
        lbIDBill.setFont(font0);
        lbIDBill.setBounds(20, 50, 120, 30);

        txtIDBill = new JTextField(hdBUS.remindMaHD());
        ...
        lbIDCustomer.setFont(font0);
        lbIDCustomer.setBounds(20, 90, 120, 30);

        txtIDCustomer = new JTextField();
        txtIDCustomer.setBounds(new Rectangle(150, 90, 240, 30));
        txtIDCustomer.setText("");
        txtIDCustomer.setEditable(true);
        txtIDCustomer.setBorder(BorderFactory.createMatteBorder(1, 1, 4, 1, new Color(0, 222, 255)));
        txtIDCustomer.setHorizontalAlignment(JTextField.CENTER);
        ...
        lbDateBill = new JLabel("Ngày hóa đơn");
...
//        scrollpane.setBorder(null);
    ...
    public String outThongTin() {
        ...
        kh = khBUS.get(txtIDCustomer.getText());
        nv = nvBUS.get(txtIDStaff.getText());

        s += "               Họ và tên khách hàng : " + kh.getHoKH().concat(" " + kh.getTenKH());
    ...
    public void clear(boolean flag) {
        if (flag) {
            // PHẦN HÓA ĐƠN
            txtIDBill.setText(hdBUS.remindMaHD());
            txtIDCustomer.setText("");
            txtIDStaff.setText("");
            if (userID != null) {
                txtIDStaff.setText(userID);
    ...
    public void blockHD(boolean flag) {
        ...
        txtIDCustomer.setEditable(!flag);
        if (userID != null) {
        ...
        if (!flag) {
            ...
            txtIDCustomer.setBorder(BorderFactory.createMatteBorder(1, 1, 4, 1, new Color(154, 154, 154)));
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource().equals(btnSubmit)) {
            if (txtIDBill.getText().isEmpty()) {
                ...
                return;
            } else if (hdBUS.check(txtIDBill.getText())) {
            ...
            if (txtIDCustomer.getText().isEmpty()) {
                JOptionPane.showMessageDialog(null, "Vui lòng nhập hoặc chọn mã khách hàng");
                txtIDCustomer.requestFocus();
                return;
            } else if (!khBUS.check(txtIDCustomer.getText())) {
        ...
        if (e.getSource().equals(btnAdd)) {
            ...
            if (flag) {
                ...
                dsct.add(new ChiTietHDDTO(txtIDBill.getText(), txtIDProduct.getText(), txtNameProduct.getText(), sl, gia));
            }
            outModel(model, dsct);
        ...
        if (e.getSource().equals(btnDelete)) {
            int messEdit = JOptionPane.showConfirmDialog(null, "Bạn có chắc chắn muốn xóa ?", "Thông báo", JOptionPane.YES_NO_OPTION);
            if (messEdit == 0) {
                reload(true);
                blockHD(false);
            }
        ...
        if (e.getSource().equals(btnConfirm)) //Xác nhận
        {
            ...
            String maKH = txtIDCustomer.getText();
    ...
    public void mouseClicked(MouseEvent e) {
        if (e.getSource() == lbIconCustomer) {
            if (lbIconCustomer.isEnabled()) {
                ListKhachHang nv = new ListKhachHang(1150, 750);
                String s = nv.getInfoKH();
                txtIDCustomer.setText(s);
            }
