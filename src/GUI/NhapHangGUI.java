...
public class NhapHangGUI extends JPanel implements MouseListener, FocusListener, DocumentListener, KeyListener {
    private final NhapHangBUS nhBUS = new NhapHangBUS();
    private final ChiTietHDBUS ctnhBUS = new ChiTietHDBUS();
    ...
    public void init() {
        ...
        lbIMG.setIcon(new ImageIcon("./img/imgBill2.png"));
...
//        lbIMG.setOpaque(true);
        ...
        listNH(); // Đọc từ database lên table
...
//        table.getColumnModel().getColumn(4).setCellRenderer(rightAlign);
    ...
    public JPanel panelInfo() {
        ...
        String[] arrInfo = {"Mã hóa dơn", "Mã nhà cung cấp", "Mã nhân viên", "Ngày phiếu nhập", "Tổng tiền"};
    ...
    public JPanel panelButton() {
        ...
        btnEdit = new JButton("Sửa phiếu nhập");
        ...
        btnDelete = new JButton("Xóa phiếu nhập");
...
//        p.add(btnIMG);
        ...
        btnBill = new JButton("In phiếu nhập");
        ...
        btnView = new JButton("Chi tiết phiếu nhập");
...
//        p.add(sepPrice);
    ...
    public void outModel(DefaultTableModel model, ArrayList<NhapHangDTO> nh) {    // Xuất ra Table từ ArrayList
        Vector data;
        model.setRowCount(0);
        for (NhapHangDTO k : nh) {
            data = new Vector();
            data.add(k.getMaNH());
            data.add(k.getMaNCC());
            data.add(k.getMaNV());
            data.add(k.getNgayNhap());
    ...
    public void listNH() {  // Chép ArrayList lên table
...
//        try {
//            if (image != null) {
//                File save = new File("./img/NhapHang/" + imgName);    //  Tạo file
...
//    }
    ...
    public void search() {
        ...
        outModel(model, nhBUS.search(ctnhBUS.getHD(sortMaSP.getText()), manv, month, year, min, max));
    }
...
//            Image newImage;
//            try {
//                newImage = new ImageIcon("./img/NhapHang/" + imgName).getImage().getScaledInstance(200, 200, Image.SCALE_DEFAULT);
//            } catch (NullPointerException E) {
//                newImage = new ImageIcon("./img/NhapHang/NoImage.jpg").getImage().getScaledInstance(200, 200, Image.SCALE_DEFAULT);
//            }
        ...
        if (e.getSource() == btnDelete) {
            ...
            if (messDelete == 0) {
                nhBUS.delete(textfields[0].getText());
                ctnhBUS.delete(textfields[0].getText());
                table.clearSelection();
                cleanView();
                listNH();   //  outModel(model, nhBUS.getList());
        ...
        if (e.getSource() == btnBill) {
            if (textfields[0].getText().equals("")) {
                JOptionPane.showMessageDialog(null, "Vui lòng chọn phiếu nhập cần in !!!");
                return;
            }
            String maNH = textfields[0].getText();
            String maNCC = textfields[1].getText();
            String maNV = textfields[2].getText();
            String ngayNH = textfields[3].getText();
            double tongTien = Double.parseDouble(textfields[4].getText().replace(",", ""));
            NhapHangDTO nh = new NhapHangDTO(maNH, maNCC, maNV, ngayNH, tongTien);
            ChiTietNHBUS ctnhBUS = new ChiTietNHBUS();
            ArrayList<ChiTietNHDTO> ctnh = ctnhBUS.getListNH(maNH);
            OutBill bill = new OutBill(nh, ctnh);
            bill.printNH();
        }
        ...
        if (e.getSource() == btnView) {
            ...
            ChiTietNHGUI chitiet = new ChiTietNHGUI(Default_Width, Default_Height, textfields[0].getText());
        }
        ...
        if (e.getSource() == btnConfirm) {
            if (AddOrEdit) {    // Add
                ...
                if (messEdit == 0) {
                    ...
                    String ngayNH = myDateObj.format(myFormatObj);

                    double tongTien = 0;

                    NhapHangDTO nh = new NhapHangDTO(maNH, maNCC, maNV, ngayNH, tongTien);
                    nhBUS.add(nh);
                    outModel(model, nhBUS.getList());   // Load lại table
                    cleanView();
                ...
                if (messEdit == 0) {
                    ...
                    NhapHangDTO nh = new NhapHangDTO(maNH, maNCC, maNV, ngayNH, tongTien);
                    nhBUS.update(nh);
                    listNH();   //  outModel(model, nhBUS.getList());   // Load lại table
                    cleanView();
...
//            String ngayNH = textfields[3].getText();
//            ctnhBUS.list();
//            double tongTien = nhBUS.updateTotalPrice(maNH, ctnhBUS.getList());
//
//            NhapHangDTO nh = new NhapHangDTO(maNH, maNCC, maNV, ngayNH, tongTien);
//            nhBUS.update(nh);
//            listNH();
