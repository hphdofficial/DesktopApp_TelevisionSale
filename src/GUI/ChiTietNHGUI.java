...
public class ChiTietNHGUI extends JDialog implements MouseListener, FocusListener, DocumentListener, KeyListener {
    private final ChiTietNHBUS ctnhBUS = new ChiTietNHBUS();
    ...
    public ChiTietNHGUI(int width, int height, String maNH) {
        Default_Width = width;
...
//        pButton.setLayout(null);
        ...
        listCTNH(); // Đọc từ database lên table
...
//        p.add(sepPrice);
    ...
    public void outModel(DefaultTableModel model, ArrayList<ChiTietNHDTO> cthd) {   // Xuất ra Table từ ArrayList
        Vector data;
        model.setRowCount(0);
        for (ChiTietNHDTO ct : cthd) {
            data = new Vector();
            data.add(ct.getMaSP());
    ...
    public void listCTNH() {    // Chép ArrayList lên table
        if (ctnhBUS.getList() == null)
            ctnhBUS.list();
        ArrayList<ChiTietNHDTO> cthd = ctnhBUS.getListNH(maNH);
        model.setRowCount(0);
        outModel(model, cthd);
    }
    ...
    public void search() {
        ...
        outModel(model, ctnhBUS.search(id, min, max));
    }
...
//            Image newImage;
//            try {
//                newImage = new ImageIcon("./img/ChiTietNH/" + imgName).getImage().getScaledInstance(200, 200, Image.SCALE_DEFAULT);
//            } catch (NullPointerException E) {
//                newImage = new ImageIcon("./img/ChiTietNH/NoImage.jpg").getImage().getScaledInstance(200, 200, Image.SCALE_DEFAULT);
//            }
        ...
        if (e.getSource() == btnDelete) {
            ...
            if (messDelete == 0) {
                ctnhBUS.deleteSP(maNH, textfields[0].getText());
                table.clearSelection();
                cleanView();
                listCTNH();   //  outModel(model, ctnhBUS.getList());
...
//                    Logger.getLogger(ChiTietNHGUI.class.getName()).log(Level.SEVERE, null, ex);
//                }
//            }
//        }
...
//
//                    ChiTietNHDTO cthd = new ChiTietNHDTO(maSP,tenSP,soLuong,donGia);
//                    ctnhBUS.add(cthd);
//                    outModel(model, ctnhBUS.getList());   // Load lại table
//                    cleanView();
...
//                }
                ...
                if (messEdit == 0) {
                    ...
                    ChiTietNHDTO cthd = new ChiTietNHDTO(maNH, maSP, tenSP, soLuong, donGia);
                    ctnhBUS.update(cthd);
                    listCTNH();   //  outModel(model, ctnhBUS.getListNH(maNH));   // Load lại table
                    cleanView();
        ...
        if (e.getSource() == btnBackSearch) {
            ...
            pSearch.setVisible(false);
            table.setEnabled(true);
            outModel(model, ctnhBUS.getListNH(maNH));
        }
